...
async function compareEtsyWithAliexpress(etsyItem, aliexpressItems, threshold = 60) {
  ...
  const comparisons = await Promise.all(
    aliexpressItems.map(async aliItem => {
      try {
        // Toujours scraper la vraie image depuis la page produit
        let realImage = null;
        if (aliItem.link) {
          realImage = await scrapeAliexpressImage(aliItem.link);
        }
        // Fallback sur l'image Serper si le scraping échoue
        const imageToUse = realImage || aliItem.image;
        // Mettre à jour l'image pour l'affichage frontend
        aliItem.image = realImage || aliItem.image;

        if (!imageToUse) {
          return aliItem.link ? { etsy: etsyItem, aliexpress: aliItem, similarity: 50, reasoning: 'Match Serper (image non disponible)' } : null;
        }

        const b64Ali = await imageUrlToBase64(imageToUse, 'AliExpress');
        if (!b64Ali) {
