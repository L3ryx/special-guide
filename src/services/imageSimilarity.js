...
async function calculateSimilarity(base64A, base64B, retries = 3) {
...
Return ONLY a decimal number between 0 and 1, nothing else.`
      ...
      if (err.response?.status === 429 && attempt < retries) {
        ...
        console.log(`⏳ OpenAI 429 — attente ${wait}ms (tentative ${attempt}/${retries})`);
...
async function compareEtsyWithAliexpress(etsyItem, aliexpressItems, threshold = 40) {
  ...
  // SÉQUENTIEL — évite le rate limit OpenAI 429
  for (const aliItem of aliexpressItems) {
    if (!aliItem.link) continue;

    try {
      // 1. Scraper la vraie image AliExpress — retourne { rawUrl, dataUrl }
      console.log(`🔎 Scrape AliExpress: ${aliItem.link.substring(0, 60)}`);
      const scraped = await scrapeAliexpressImage(aliItem.link);
      if (!scraped) { console.log(`⚠️ Image non trouvée, skip`); continue; }

      const { rawUrl, dataUrl } = scraped;

      // 2. Upload l'URL directe alicdn sur ImgBB (pas le webp base64)
      console.log(`📤 Upload AliExpress sur ImgBB...`);
      const imgbbUrl = await uploadToImgBB(rawUrl);
      console.log(`🔗 ImgBB: ${imgbbUrl.substring(0, 60)}`);

      // 3. Comparer avec OpenAI en base64
      const b64Ali = dataUrl.split(',')[1];
      if (!b64Ali) continue;

      // Délai entre chaque appel OpenAI pour éviter rate limit
      ...
      // Stocker l'URL ImgBB pour l'affichage
      candidates.push({
        etsy: etsyItem,
      ...
      console.error(`Erreur candidat: ${err.message}`);
    }
  }
