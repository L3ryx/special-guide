/**
 * etsyScraper.js
 * Scrape les pages de résultats Etsy directement (sans navigateur).
 *
 * Flux :
 *  1. searchEtsyPages(keyword, maxPages)
 *     → GET https://www.etsy.com/search?q=...&page=N
 *     → Parse chaque carte listing : listingId, image, shopName, shopUrl
 *
 *  2. getSecondShopImage(shopUrl, excludeListingId)
 *     → GET https://www.etsy.com/shop/<name>
 *     → Retourne l'image d'un 2ème listing différent de la boutique
 *
 * Dépendances : cheerio
 */
...
class RateLimiter {
  ...
  async wait(minMs = 800, maxMs = 2000) {
...
function parseEtsySearchPage(html) {
  ...
  // Méthode 1 : JSON-LD structured data
  ...
  if (results.length < 5) {
    ...
    $(cardSelectors).each(function(_, card) {
      ...
      const listingId =
        ...
        $card.attr('data-palette-listing-id') || null;
...
// ── Parser : page boutique → 2ème image ──────────────────────────────────────

/**
 * Visite la page boutique Etsy et retourne l'image d'un listing
 * différent de excludeListingId.
 *
 * @param {string} shopUrl            - URL complète de la boutique
 * @param {string} excludeListingId   - listingId du 1er listing (à ignorer)
 * @returns {Promise<string|null>}    - URL image ou null
 */
async function getSecondShopImage(shopUrl, excludeListingId) {
  if (!shopUrl) return null;
  try {
    await _shopRateLimiter.wait(600, 1500);
    ...
    if (image2) {
      console.log('[etsyScraper] 2ème image boutique OK: ' + shopUrl.slice(0, 60));
    } else {
      console.log('[etsyScraper] Pas de 2ème image pour: ' + shopUrl.slice(0, 60));
    }
    return image2;
  } catch (e) {
...
/**
 * Scrappe jusqu'à maxPages pages de résultats Etsy pour le keyword.
 * Déduplique par boutique (1 listing par boutique).
 *
 * @param {string}   keyword
 * @param {number}   maxPages
 * @param {Function} onPage     callback(pageNum, totalShopsFound)
 * @param {Function} isAborted  () => bool
 * @returns {Promise<Array>}    listings uniques avec image
 */
async function searchEtsyPages(keyword, maxPages, onPage, isAborted) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    if (page > 1) await _rateLimiter.wait(1200, 3000);
    ...
    if (pageResults.length === 0) {
      console.log('[etsyScraper] Page vide — arrêt de la pagination');
      break;
    }
  }
...
async function searchListingIds(keyword, limit, offset) {
  ...
  await _rateLimiter.wait(800, 2000);
...
async function getShopInfo(shopIdOrName) {
  return { shopId: null, shopName: String(shopIdOrName), title: String(shopIdOrName), shopUrl: 'https://www.etsy.com/shop/' + shopIdOrName, shopAvatar: null, numSales: 0, admirers: 0 };
}
