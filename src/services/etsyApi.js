...
async function getShopNameAndImage(shopId, listingId, listingId2 = null, listingId3 = null, listingId4 = null) {
  ...
  // 2. Fetch images for all listing IDs in parallel
  async function fetchImage(lid, index) {
    if (!lid) return null;
    try {
      ...
      console.warn(`[etsyApi] image${index} failed for listing`, lid, ':', e.message);
      return null;
    }
  }

  const [image, image2, image3, image4] = await Promise.all([
    fetchImage(listingId,  1),
    fetchImage(listingId2, 2),
    fetchImage(listingId3, 3),
    fetchImage(listingId4, 4),
  ]);
