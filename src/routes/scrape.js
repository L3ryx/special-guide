...
router.post('/search', async (req, res) => {
  ...
  try {
    ...
    send('comparing', `🤖 Comparaison IA sur ${withMatches.length} annonce(s) avec résultats...`);

    // ÉTAPE 3 : Comparaison IA — toutes en parallèle (3 à la fois)
    let compareDone = 0;
    const allResults = [];

    await parallel(
      withMatches,
      3, // 3 comparaisons OpenAI simultanées
      async ({ listing, matches }) => {
        try {
          const comparisons = await compareEtsyWithAliexpress(listing, matches, similarityThreshold);
          compareDone++;
          if (comparisons.length > 0) {
            allResults.push(...comparisons);
            send('match_found', `✅ ${comparisons.length} match(s) trouvé(s) — ${allResults.length} total`);
          } else {
            send('comparing', `🤖 Comparaison ${compareDone}/${withMatches.length} — aucun match`);
          }
        } catch (err) {
    ...
    sendComplete(allResults.sort((a, b) => b.similarity - a.similarity));
