...
router.post('/clone', requireAuth, async (req, res) => {
  ...
  async function sbFetch(url) {
    const key = process.env.SCRAPEAPI_KEY;
    if (!key) throw new Error('SCRAPEAPI_KEY not configured');
    const sessionNumber = Math.floor(Math.random() * 9999);
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const r = await axios.get('https://api.scraperapi.com', {
          params: { api_key: key, url, render: 'false', country_code: 'us', session_number: sessionNumber, keep_headers: 'true' },
          timeout: 70000,
        });
        ...
        if (e.response?.status === 401) throw new Error('SCRAPEAPI_KEY invalid (401)');
        if (e.response?.status === 403) throw new Error('ScraperAPI credits exhausted (403)');
        if (attempt === 3) throw new Error('ScraperAPI failed: ' + e.message);
